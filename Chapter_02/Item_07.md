...
```
* 위 코드에서 `pop()` 메소드는 스택에서 요소 제거 후, 해당 요소에 대한 참조를 `null`로 설정하여 메모리 누수를 방지함. <br>
이로써, 가비지컬렉터(GC)가 더 이상 필요치 않은 객체를 회수할 수 있게 됨. <br><br>

### 정리하자면, 아래와 같다.
1) 필요 없어진 객체 참조를 해제하지 않으면 메모리 누수가 발생할 수 있음 (GC가 인스턴스를 heap 해제 하지 않기 때문)
